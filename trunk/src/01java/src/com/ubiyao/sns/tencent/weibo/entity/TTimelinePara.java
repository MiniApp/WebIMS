 ...
 */
public class TTimelinePara implements Serializable {
    ...
    public int transformPageToPageFlag(int page, boolean pageDown) {
        if (page <= 0) {
            return TConstant.VALUE_FIRST_PAGE;
        }
        if (pageDown) {
            return (page == 1) ? TConstant.VALUE_FIRST_PAGE : TConstant.VALUE_NEXT_PAGE;
        } else {
            return (page == 1) ? TConstant.VALUE_FIRST_PAGE : TConstant.VALUE_LAST_PAGE;
        }
    }

    /**
     * 得到复合类型的statusType
     * multiStatusType({@link TConstant#VALUE_STATUS_TYPE_COMMENT}, {@link TConstant#VALUE_STATUS_TYPE_Mention})
     * 
     ...
     * @return
     */
    public TTimelinePara copy(TTimelinePara source) {
        TTimelinePara destin = new TTimelinePara();
    ...
    public Map<String, String> getParasMap() {
        ...
        if (pageTime >= 0) {
            parasMap.put(TConstant.PARA_PAGE_TIME, Long.toString(pageTime));
        }
        if (lastId >= 0) {
        ...
        if (statusType >= 0) {
            parasMap.put(TConstant.PARA_STATUS_TYPE, Integer.toString(statusType));
        }
        if (contentType >= 0) {
            parasMap.put(TConstant.PARA_CONTENT_TYPE, Integer.toString(contentType));
        }
        if (accessLevel >= 0) {
            parasMap.put(TConstant.PARA_ACCESS_LEVEL, Integer.toString(accessLevel));
        }
        if (position >= 0) {
            parasMap.put(TConstant.PARA_POSITION, Long.toString(position));
        }
        if (repostOrCommentFlag >= 0) {
            parasMap.put(TConstant.PARA_REPOST_OR_COMMENT_FLAG, Long.toString(repostOrCommentFlag));
        }
        if (rootId >= 0) {
            parasMap.put(TConstant.PARA_ROOT_ID, Long.toString(rootId));
        }
        if (twitterId >= 0) {
            parasMap.put(TConstant.PARA_TWITTER_ID, Long.toString(twitterId));
        }
        if (nextTime >= 0) {
            parasMap.put(TConstant.PARA_NEXT_TIME, Long.toString(nextTime));
        }
        if (prevTime >= 0) {
            parasMap.put(TConstant.PARA_PREV_TIME, Long.toString(prevTime));
        }
        return parasMap;
    }
