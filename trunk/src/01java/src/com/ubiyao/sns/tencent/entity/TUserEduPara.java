 ...
 */
public class TUserEduPara implements Serializable {
    ...
    public Map<String, String> getParasMap() {
        ...
        if (schoolId >= 0) {
            parasMap.put(TConstant.PARA_USER_EDU_SCHOOL_ID, Long.toString(schoolId));
        }
        if (departmentId >= 0) {
            parasMap.put(TConstant.PARA_USER_EDU_DEPARTMENT_ID, Long.toString(departmentId));
        }
        if (level >= 0) {
            parasMap.put(TConstant.PARA_USER_EDU_LEVEL, Long.toString(level));
        }
        return parasMap;
    }
