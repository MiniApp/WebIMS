...
public class THttpClient {
    ...
    // 日志输出
    private static Log log = LogFactory.getLog(THttpClient.class);
     ...
     */
    public THttpClient(int maxConnectionsPerHost, int maxTotalConnections, int conTimeOutMs, int soTimeOutMs, List<RouteCfg> routeCfgList, HttpHost proxy) {
    ...
    public String httpPostWithFile(String url, String queryString, List<NameValuePair> files) throws Exception {
        ...
        List<NameValuePair> queryParamList = TStrOperate.getQueryParamsList(queryString);
        ...
        for (NameValuePair param : files) {
            ...
            log.info("---------- File Path = " + filePath + "\n---------------- MIME Types = "
                    + THttpUtil.getContentType(targetFile));
            fileBody = new FileBody(targetFile, THttpUtil.getContentType(targetFile), "UTF-8");
